...
export default class ChildProcess extends Process {
    ...
    public async initialize(): Promise<void> {
        ...
        const pathToExe: string = this.getSettings().findSetting("ghd_path").getValue() as string;
        ...
        const response: DataResponse = await this.requestExternal('aarontburn.Monkey_Core', 'add-window', {
            ...
            filter: (w: Window) => w.getTitle() === "GitHub Desktop" && w.isVisible(),
            callback: this.onMonkeyEvent.bind(this)

        } as MonkeyParams);

        if (response.code === HTTPStatusCodes.NOT_FOUND) {
            console.error(`[Github Desktop Monkey] Missing dependency: Monkey Core (aarontburn.Monkey_Core) https://github.com/aarontburn/nexus-monkey-core`);
    ...
    public registerSettings(): (Setting<unknown> | string)[] {
        return [
            new StringSetting(this)
                .setDefault('')
                .setName("Github Desktop Executable Path")
                .setDescription("The path to your Github Desktop executable file. Restart required.")
                .setAccessID('ghd_path')
                .setValidator(s => {
                    return (s as string).replace(/\\\\/g, '/')
                }),

            new BooleanSetting(this)
                .setName("Close Github Desktop on Exit")
                .setDefault(false)
                .setDescription("This will only work when Github Desktop is opened through Github Desktop Monkey. Restart required.")
    ...
    public async onSettingModified(modifiedSetting?: Setting<unknown>): Promise<void> {
        if (modifiedSetting?.getAccessID() === 'ghd_path') {
